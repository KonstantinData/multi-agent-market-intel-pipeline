...
PROMPT = """Identify supply chain technology (ERP, MES, WMS, planning tools).
Look for integration and platform evidence in sources.
Return only verified signals; use n/v when unclear.
"""
...
def _extract_tech_lines(text: str) -> List[str]:
    keywords = [
        "erp",
        "mes",
        "wms",
...
class AgentAG70SupplyChainTech(BaseAgent):
    ...
    def run(self, case_input: Dict[str, Any]) -> AgentResult:
        ...
        tech_lines: List[Tuple[str, str, str]] = []
        for page in pages:
            for line in _extract_tech_lines(page.text):
                tech_lines.append((line, page.url, page.publisher))

        tech_lines = tech_lines[:6]
        ...
        if not tech_lines:
            findings_notes.append(
                "No verifiable supply chain technology stack evidence found in company or news sources (n/v)."
            )
        else:
        ...
        output: Dict[str, Any] = {
            ...
            "findings": [
                {
                    "summary": "Supply chain technology signals",
