...
def _build_sources(company_name: str, domain: str) -> List[Dict[str, str]]:
    ...
    external_urls = [
        ...
        ("Google News", f"https://news.google.com/search?q={company_query}%20funding"),
        ("PR Newswire", f"https://www.prnewswire.com/search/news/?keyword={company_query}%20funding"),
        (
            "Business Wire",
            f"https://www.businesswire.com/portal/site/home/search/?searchTerm={company_query}%20financial",
        ),
        ("MarketScreener", f"https://www.marketscreener.com/search/?q={company_query}"),
    ]
...
class AgentAG21FinancialSignals(BaseAgent):
    ...
    def run(self, case_input: Dict[str, Any]) -> AgentResult:
        ...
        financial_lines: List[Tuple[str, str, str]] = []
        for page in pages:
            for line in _extract_financial_lines(page.text):
                financial_lines.append((line, page.url, page.publisher))

        financial_lines = financial_lines[:6]
        ...
        if not financial_lines:
            findings_notes.append(
                "No verifiable financial development signals found in company filings or news sources (n/v)."
            )
        else:
            for line, url, _publisher in financial_lines:
                findings_notes.append(f"Financial signal: {line} (source: {url}).")
        ...
        output: Dict[str, Any] = {
            ...
            "findings": [
                {
                    "summary": "Financial development signals",
