...
def _build_sources(company_name: str, domain: str) -> List[Dict[str, str]]:
    ...
    external_urls = [
        ("Google News", f"https://news.google.com/search?q={company_query}%20industry"),
        ("PR Newswire", f"https://www.prnewswire.com/search/news/?keyword={company_query}%20market"),
        (
            "Business Wire",
            f"https://www.businesswire.com/portal/site/home/search/?searchTerm={company_query}%20industry",
        ),
    ]
...
def _extract_market_lines(text: str) -> List[str]:
    keywords = [
        "industry",
        "market",
        "sector",
...
class AgentAG31MarketsFocus(BaseAgent):
    ...
    def run(self, case_input: Dict[str, Any]) -> AgentResult:
        ...
        market_lines: List[Tuple[str, str, str]] = []
        for page in pages:
            for line in _extract_market_lines(page.text):
                market_lines.append((line, page.url, page.publisher))

        market_lines = market_lines[:6]
        ...
        if not market_lines:
            findings_notes.append(
                "No verifiable market or industry focus statements found in company or news sources (n/v)."
            )
        else:
            for line, url, _publisher in market_lines:
                findings_notes.append(f"Market focus signal: {line} (source: {url}).")
        ...
        output: Dict[str, Any] = {
            ...
            "findings": [
                {
                    "summary": "Markets and industry focus",
