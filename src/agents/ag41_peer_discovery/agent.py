...
PROMPT = """Identify peer/competitor manufacturers with similar offerings.
Provide evidence of overlap (products, markets, capabilities).
Only include peers with sourced justification.
"""
...
def _build_sources(company_name: str, domain: str) -> List[Dict[str, str]]:
    ...
    external_urls = [
        ("SimilarWeb", f"https://www.similarweb.com/website/{domain}/competitors/"),
        ("Google News", f"https://news.google.com/search?q={company_query}%20competitor"),
        ("PR Newswire", f"https://www.prnewswire.com/search/news/?keyword={company_query}%20competitor"),
        (
            "Business Wire",
            f"https://www.businesswire.com/portal/site/home/search/?searchTerm={company_query}%20competitor",
        ),
    ]
...
def _extract_peer_lines(text: str) -> List[str]:
    keywords = [
        "competitor",
        "competitors",
        "peer",
...
class AgentAG41PeerDiscovery(BaseAgent):
    ...
    def run(self, case_input: Dict[str, Any]) -> AgentResult:
        ...
        peer_lines: List[Tuple[str, str, str]] = []
        for page in pages:
            for line in _extract_peer_lines(page.text):
                peer_lines.append((line, page.url, page.publisher))

        peer_lines = peer_lines[:6]
        ...
        if not peer_lines:
            findings_notes.append(
                "No verifiable peer or competitor references found in company or public sources (n/v)."
            )
        else:
            for line, url, _publisher in peer_lines:
                findings_notes.append(f"Peer discovery signal: {line} (source: {url}).")
        ...
        output: Dict[str, Any] = {
            ...
            "findings": [
                {
                    "summary": "Peer and competitor discovery",
