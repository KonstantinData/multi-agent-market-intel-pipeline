...
def _build_sources(company_name: str, domain: str) -> List[Dict[str, str]]:
    ...
    external_urls = [
        (
            "Google News",
            f"https://news.google.com/search?q={company_query}%20supply%20chain%20risk",
        ),
        (
            "PR Newswire",
            f"https://www.prnewswire.com/search/news/?keyword={company_query}%20disruption",
        ),
        (
            "Business Wire",
    ...
    def run(self, case_input: Dict[str, Any]) -> AgentResult:
        ...
        if not risk_lines:
            ...
            for line, url, _publisher in risk_lines:
                findings_notes.append(
                    f"Supply chain risk signal: {line} (source: {url})."
                )
