...
def _build_sources(company_name: str, domain: str) -> List[Dict[str, str]]:
    ...
    external_urls = [
        ...
        ("Google News", f"https://news.google.com/search?q={company_query}%20industry%20trend"),
        ("PR Newswire", f"https://www.prnewswire.com/search/news/?keyword={company_query}%20industry"),
        ("Business Wire", f"https://www.businesswire.com/portal/site/home/search/?searchTerm={company_query}%20market"),
...
def _extract_trend_lines(text: str) -> List[str]:
    keywords = [
        "market",
        "industry",
        "trend",
...
class AgentAG81IndustryTrends(BaseAgent):
    ...
    def run(self, case_input: Dict[str, Any]) -> AgentResult:
        ...
        trend_lines: List[Tuple[str, str, str]] = []
        for page in pages:
            for line in _extract_trend_lines(page.text):
                trend_lines.append((line, page.url, page.publisher))

        trend_lines = trend_lines[:6]
        ...
        if not trend_lines:
            findings_notes.append(
                "No verifiable industry trend signals found in publications, event listings, or company sources (n/v)."
            )
        else:
            for line, url, _publisher in trend_lines:
                findings_notes.append(f"Trend signal: {line} (source: {url}).")
