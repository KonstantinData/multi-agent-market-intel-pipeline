...
def _build_sources(company_name: str, domain: str) -> List[Dict[str, str]]:
    ...
    external_urls = [
        ("Google News", f"https://news.google.com/search?q={company_query}%20product%20launch"),
        ("PR Newswire", f"https://www.prnewswire.com/search/news/?keyword={company_query}%20product"),
        (
            "Business Wire",
            f"https://www.businesswire.com/portal/site/home/search/?searchTerm={company_query}%20solution",
        ),
    ]
...
def _extract_portfolio_lines(text: str) -> List[str]:
    keywords = [
        "product",
        "service",
        "solution",
    ...
    for line in text.split("\n"):
        ...
        if not line_text:
            continue
        if len(line_text) < 35 or len(line_text) > 240:
            continue
...
class AgentAG30Portfolio(BaseAgent):
    ...
    def run(self, case_input: Dict[str, Any]) -> AgentResult:
        ...
        portfolio_lines: List[Tuple[str, str, str]] = []
        for page in pages:
            for line in _extract_portfolio_lines(page.text):
                portfolio_lines.append((line, page.url, page.publisher))

        portfolio_lines = portfolio_lines[:6]
        ...
        if not portfolio_lines:
            findings_notes.append(
                "No verifiable portfolio evidence found in official pages or product announcements (n/v)."
            )
        else:
            for line, url, _publisher in portfolio_lines:
                findings_notes.append(f"Portfolio signal: {line} (source: {url}).")
